...
export class Whispers {
  ...
  setFlames() {
    ...
    this.revealBuffer = new THREE.StorageInstancedBufferAttribute(
      this.revealArray,
      1,
    );
    this.revealBufferNeedsUpdate = true;

    const revealAttribute = storage(
      this.revealBuffer,
      "float",
      this.count,
    ).toAttribute();
    ...
    beamMaterial.positionNode = Fn(() => {
      ...
      const noiseUv = vec2(
        random,
        uv().y.mul(0.5).sub(this.game.ticker.elapsedScaledUniform.mul(0.1)),
      );
      const noise = texture(this.game.noises.perlin, noiseUv)
        .r.sub(0.5)
        .mul(noiseStrength);
    ...
    beamMaterial.outputNode = Fn(() => {
      ...
      const mask = texture(
        this.game.resources.whisperFlameTexture,
        baseUv,
      ).r.sub(revealAttribute.oneMinus());
...
{
    // initial load
    if(data.type === 'init' && data.whispers)
    {
        for(const whisper of data.whispers)
            this.data.insert(whisper)
    }

    // single whisper added (live)
    else if(data.type === 'whisperAdded' && data.whisper)
    {
        this.data.insert(data.whisper)
    }

    // delete
    else if(data.type === 'whispersDelete' && data.whispers)
    {
        for(const whisper of data.whispers)
  ...
  setMenu() {
    ...
    this.menu.previewMessage = this.menu.instance.previewElement.querySelector(
      ".js-preview-message",
    );
    this.menu.previewMessageText =
      this.menu.previewMessage.querySelector(".js-text");
    this.menu.previewMessageFlag =
      this.menu.previewMessage.querySelector(".js-flag");
    ...
    const submit = () => {
      ...
      if (sanatized.length && this.game.server.connected) {
        ...
        this.game.server.send({
          type: "whispersInsert",
          message: sanatized,
          countryCode: this.menu.inputFlag.country
            ? this.menu.inputFlag.country.code
            : "",
    ...
    this.menu.input.addEventListener("input", () => {
      ...
      this.menu.previewMessageText.textContent = sanatized.length
        ? sanatized
        : "Your message here";
    ...
    this.menu.previewMessageText.addEventListener("input", (event) => {
      const sanatized = sanatize(
        this.menu.previewMessageText.textContent,
        false,
        true,
        true,
      );
    ...
    this.menu.previewMessageText.addEventListener("blur", () => {
      const sanatized = sanatize(this.menu.input.value, true, true, true);
      this.menu.previewMessageText.textContent =
        sanatized !== "" ? sanatized : "Your message here";
      updateGroup();
    });
     ...
     * Flag
     */
    this.menu.inputFlag = new InputFlag(
      this.menu.inputGroup.querySelector(".js-input-flag"),
    );
    ...
    if (closestDistance < this.bubble.minDistance) {
      if (closestWhisper !== this.bubble.closest) {
        ...
        if (closestWhisper.countryCode) {
          const country = this.menu.inputFlag.countries.get(
            closestWhisper.countryCode,
          );

          if (country) imageUrl = country.imageUrl;
        }

        this.bubble.instance.tryShow(
          closestWhisper.message,
          position,
          imageUrl,
        );
