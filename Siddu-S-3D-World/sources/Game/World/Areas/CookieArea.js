...
export class CookieArea extends Area {
  ...
  setCounter() {
    ...
    this.game.server.events.on("message", (data) => {
      // initial values when connecting
      if (data.type === "globalsInit") {
        this.counter.value = data.cookies;
        this.counter.update();
      }

      // live updates
      if (data.type === "cookiesUpdate") {
        this.counter.value = data.total;
        this.counter.update();
      }
    });

    // Message already received
    if(this.game.server.initData?.cookies !== undefined)
{
    this.counter.value = this.game.server.initData.cookies
}
