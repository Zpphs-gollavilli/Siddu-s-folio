...
export class AltarArea extends Area {
  constructor(model) {
    ...
    if (this.game.debug.active) {
      ...
      this.debugPanel.addBinding(this.emissive, "value", {
        label: "emissive",
        min: 0,
        max: 10,
        step: 0.1,
  ...
  setBeam() {
    ...
    const bottomMaterial = new THREE.MeshBasicNodeMaterial({
      transparent: true,
    });
  ...
  setGlyphs() {
    ...
    const emissiveAMaterial = this.game.materials.getFromName(
      "emissiveBlueRadialGradient",
    );
    const emissiveBMaterial = this.game.materials.getFromName(
  ...
  setData() {
    ...
    this.game.server.events.on("message", (data) => {
      // first sync when connecting
      if (data.type === "globalsInit") {
        this.updateText(data.altar);
        return;
      }

      // live updates
      if (data.type === "altarUpdate") {
        this.updateText(data.total);
        return;
      }
    });

    // Init already received before this area was created
    if (this.game.server.initData?.altar !== undefined) {
      this.updateText(this.game.server.initData.altar);
    }
  }
